...
export async function connectToDatabase() {
    ...
    const postsCollection = db.collection<Post>(process.env.POST_COLLECTION_NAME);
...
async function applySchemaValidation(db: mongoDB.Db) {
    ...
    await db.command({
        ...
        if (error.codeName === 'NamespaceNotFound') {
            await db.createCollection(process.env.POST_COLLECTION_NAME, {validator: jsonSchema});
        }
    });
}
